...
export function QAView() {
  ...
  const fetchSprints = async () => {
    setIsLoading(true);
    try {
      const sprintsData = await api.getSprints(projectId!);
  ...
  const fetchSprintCards = async (sprintId: string) => {
    ...
    try {
      const cards = await api.getCardsBySprint(sprintId);
  ...
  const fetchTroubleReport = async () => {
    ...
    setIsLoading(true);
    try {
      const report = await api.getTroubleReport(projectId, selectedSprintId);
