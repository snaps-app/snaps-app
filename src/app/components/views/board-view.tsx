...
import { EpicModal } from '@/app/components/board/EpicModal';
import { SprintModal } from '@/app/components/board/SprintModal';
import { BulkApplyModal } from '@/app/components/board/BulkApplyModal';
import { VaccinationModal } from '@/app/components/board/VaccinationModal';
import { PlannerPanel } from '@/app/components/board/PlannerPanel';
...
export function BoardView() {
  ...
  const handleSaveBoard = async () => {
    ...
    try {
      if (localBoardId) {
        const updated = await updateBoard(localBoardId, { name: boardName, code: boardCode, color: boardColor, columns: board?.columns || [] });
        setBoard(updated);
  ...
  const handleOpenBulkApply = async () => {
    ...
    try {
      ...
      const results = await Promise.all(projects.map(async p => (await getProjectBoards(p.id)).map(b => ({ ...b, projectName: p.name }))));
  ...
  const handleMove = async (taskId: string, newStatus: string) => {
    ...
    try {
      await updateCardStatus(taskId, newStatus);
  ...
  const onCreateSprint = async () => {
    ...
    try {
      await handleCreateSprint(sprintNameInput, sprintTagInput, sprintObjectiveInput);
      setSprintNameInput('');
      setSprintTagInput('');
  ...
  const onUpdateSprint = async (id: string) => {
    setIsSprintSaving(true);
    try {
      await handleUpdateSprint(id, sprintNameInput, sprintTagInput, sprintObjectiveInput);
