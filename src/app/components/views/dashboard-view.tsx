import { cloneYesterdayExecutions, getAllDailyExecutions, getDashboardStats } from '@/services/dailyExecutions';
import { applyMigrations } from '@/services/migrations';
import { getRoutinesForDate, setRoutineCompletion } from '@/services/routines';
import type { DailyExecutionWithProject, DashboardStats, Routine, RoutineWithStatus } from '@/services/types';
...
export function DashboardView() {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const [statsData, executionsData, routinesData] = await Promise.all([
                getDashboardStats(),
                getAllDailyExecutions(0, 100, todayStr),
                getRoutinesForDate(todayStr)
            ]);
    ...
    const handleToggleRoutineStatus = async (routineId: string, date: string, newStatus: string) => {
        ...
        const todayStr = format(new Date(), 'yyyy-MM-dd');
        const updated = await getRoutinesForDate(todayStr);
        setRoutines(updated);
    };
